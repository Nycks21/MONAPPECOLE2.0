...
function loadMatiere() {
    fetch("api/MatiereList.aspx")
        .then(safeJson)
        .then(data => {
            if (!Array.isArray(data)) return;

            const filteredData = data; // filtrage éventuel
            ...
            if (!table) {
                table = $('#ListTable').DataTable({
                    ...
                    order: [[0, 'asc']],
                    columns: [
                        { data: 'LABEL', title: 'Matière', className: 'text-left' },
                        {
                            data: null,
                            title: 'Actions',
                    ...
                    `,
                    language: {
                        zeroRecords: "Aucune matière trouvée",
                        search: " ",
                        searchPlaceholder: "Taper pour rechercher...",
                        lengthMenu: "Afficher _MENU_ matières",
        ...
        .catch(err => {
            console.error(err);
            alert("Erreur chargement des matières : " + err.message);
        });
}
...
// ============================
window.loadMatiere = loadMatiere;
