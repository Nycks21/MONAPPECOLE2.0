    ...
    fetch("~/dashboard.cs")
        ...
        .then(data => {
            ...
            if (!table) {
                ...
                $('#addUserModal thead tr:eq(1) th').each(function (i) {
                    if (i === 5) return;
...
function updateElevesStats() {
    ...
    const totalElevesStat = document.getElementById('totalElevesStat');
    const elevesActifsStat = document.getElementById('elevesActifsStat');
    const elevesInactifsStat = document.getElementById('elevesInactifsStat');
    const nouveauxElevesStat = document.getElementById('nouveauxElevesStat');
    
    // Si les éléments n'existent pas, on sort silencieusement
    if (!totalElevesStat && !elevesActifsStat && !elevesInactifsStat && !nouveauxElevesStat) {
        console.warn('updateElevesStats: Éléments de statistiques non trouvés');
        return;
    }
    ...
    // CORRECTION: Ne pas modifier l'objet Date original
    const today = new Date();
    const thirtyDaysAgo = new Date(today);
    thirtyDaysAgo.setDate(today.getDate() - 30);
    const nouveaux = elevesData.filter(e => new Date(e.dateInscription) >= thirtyDaysAgo).length;
    
    // Mettre à jour chaque élément individuellement avec vérification
    if (totalElevesStat) totalElevesStat.textContent = total;
    if (elevesActifsStat) elevesActifsStat.textContent = actifs;
    if (elevesInactifsStat) elevesInactifsStat.textContent = inactifs;
    if (nouveauxElevesStat) nouveauxElevesStat.textContent = nouveaux;
}
...
function filterElevesTable() {
    ...
    const filterClasseSelect = document.getElementById('filterClasseEleve');
    const filterStatutSelect = document.getElementById('filterStatutEleve');
    
    // Vérifier que les filtres existent
    if (!searchInput && !filterClasseSelect && !filterStatutSelect) {
        console.warn('filterElevesTable: Filtres non trouvés');
        return;
    }
    
    const searchTerm = searchInput ? searchInput.value.toLowerCase() : '';
    const filterClasse = filterClasseSelect ? filterClasseSelect.value : '';
    const filterStatut = filterStatutSelect ? filterStatutSelect.value : '';
    
    filteredElevesData = elevesData.filter(e => {
        const matchSearch = e.nom.toLowerCase().includes(searchTerm) || 
                           e.matricule.toLowerCase().includes(searchTerm) || 
                           e.classe.toLowerCase().includes(searchTerm);
        const matchClasse = !filterClasse || e.classe.includes(filterClasse);
        
        // CORRECTION: Gérer correctement le filtre 'nouveaux'
