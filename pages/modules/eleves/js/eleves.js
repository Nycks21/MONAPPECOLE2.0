...
// Met à jour l'affichage du nom du fichier sélectionné
function updateFileName() {
    const fileInput = document.getElementById('excelFile');
    const displayInput = document.getElementById('fileNameDisplay');
    const btnLaunch = document.getElementById('btnLaunchImport');
...
// Envoie le fichier et le mappage au serveur
function launchImport() {
    ...
    // Construction de l'objet de mappage
    const mapping = {};
    document.querySelectorAll('#mappingTableBody input').forEach(input => {
        mapping[input.getAttribute('data-field')] = input.value;
    });

    const formData = new FormData();
    formData.append("file", file);
    // CORRECTION : Utilisation de la virgule (,) et non du deux-points (:)
    formData.append("mapping", JSON.stringify(mapping));
    formData.append("modele", document.getElementById('importModele').value);
    ...
    .then(res => {
        if (res.success) {
            Swal.fire("Succès", `${res.count} élèves importés avec succès.`, "success");
            loadEleves(); 
            closeModal('modalImport');
        } else {
            Swal.fire({
                title: "Erreurs de données",
                html: `<div style="text-align:left; max-height:200px; overflow-y:auto;">${res.errors.join('<br>')}</div>`,
                icon: "warning"
            });
        }
    })
    .catch(err => {
        ...
        Swal.fire("Erreur", "Le serveur n'a pas pu traiter le fichier.", "error");
    })
...
window.saveMapping = function() {
    // Logique pour sauvegarder le mappage (ex: dans localStorage)
