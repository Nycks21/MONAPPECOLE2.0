<%@ Page Language="C#" AutoEventWireup="true" CodeFile="utilitaires.cs" Inherits="utilitaires" %>
    ...
    <head>
        ...
        <style>
        /* ── Wizard steps ────────────────────────── */
        .imp-wizard { display:flex; align-items:center; gap:0; margin-bottom:32px; }
        .imp-step { display:flex; align-items:center; gap:10px; flex:1; position:relative; }
        .imp-step:not(:last-child)::after {
            content:''; position:absolute; top:20px; left:calc(40px + 12px);
            width:calc(100% - 70px); height:2px; background:#dee2e6; z-index:0;
        }
        .imp-step.done::after  { background:#28a745; }
        .imp-step.active::after { background:#dee2e6; }
        .imp-step-circle {
            width:40px; height:40px; border-radius:50%; display:flex; align-items:center;
            justify-content:center; font-weight:700; font-size:15px; z-index:1; flex-shrink:0;
            border:2px solid #dee2e6; background:#fff; color:#adb5bd; transition:all .3s;
        }
        .imp-step.active .imp-step-circle { border-color:#007bff; background:#007bff; color:#fff; }
        .imp-step.done   .imp-step-circle { border-color:#28a745; background:#28a745; color:#fff; }
        .imp-step-label { font-size:12px; font-weight:600; color:#adb5bd; line-height:1.3; }
        .imp-step.active .imp-step-label { color:#007bff; }
        .imp-step.done   .imp-step-label { color:#28a745; }

        /* ── Dropzone ────────────────────────────── */
        .imp-dropzone {
            border:2px dashed #ced4da; border-radius:12px; padding:40px;
        ...
        .imp-dropzone i { font-size:48px; color:#007bff; margin-bottom:12px; }
        #imp-file-name { font-weight:600; color:#212529; margin:8px 0 4px; }
        #imp-file-status { font-size:13px; }

        /* ── Colonnes détectées ──────────────────── */
        #imp-columns-preview {
            display:none; flex-wrap:wrap; gap:6px; margin-top:14px;
        }
        .col-badge {
            background:#e9ecef; border-radius:20px; padding:4px 12px;
            font-size:12px; border:1px solid #dee2e6;
        }
        .col-badge strong { color:#007bff; margin-right:4px; }

        /* ── Mapping ─────────────────────────────── */
        .imp-mapping-table { width:100%; border-collapse:collapse; }
        .imp-mapping-table th { background:#343a40; color:#fff; padding:10px 12px; font-size:13px; }
        .imp-mapping-table td { padding:8px 12px; border-bottom:1px solid #dee2e6; font-size:13px; vertical-align:middle; }
        .imp-mapping-table tr:hover td { background:#f8f9fa; }
        .champ-label { font-weight:600; display:block; }
        .champ-label.required::after { content:' *'; color:#dc3545; }
        .champ-hint { color:#aaa; font-size:11px; display:block; margin-top:2px; }
        .preview-val { color:#007bff; font-style:italic; font-size:12px; }

        /* ── Stats aperçu ────────────────────────── */
        .imp-stats-bar { display:flex; gap:16px; margin-bottom:16px; flex-wrap:wrap; }
        .imp-stat { display:flex; align-items:center; gap:8px; padding:10px 18px;
                    border-radius:8px; font-size:14px; font-weight:600; }
        .imp-stat.ok    { background:#d4edda; color:#155724; }
        .imp-stat.err   { background:#f8d7da; color:#721c24; }
        .imp-stat.total { background:#e2e3e5; color:#383d41; }

        /* ── Tableau aperçu / résultat ───────────── */
        .imp-table { width:100%; border-collapse:collapse; min-width:700px; font-size:12px; }
        .imp-table th { background:#343a40; color:#fff; padding:8px 10px; white-space:nowrap; }
        .imp-table td { padding:7px 10px; border-bottom:1px solid #dee2e6; white-space:nowrap; }
        .row-valid td { background:#fff; }
        .row-error td { background:#fff8f8; }
        .row-warn  td { background:#fffbf0; }
        .badge-ok  { background:#28a745; color:#fff; padding:3px 8px; border-radius:10px; font-size:11px; }
        .badge-err { background:#dc3545; color:#fff; padding:3px 8px; border-radius:10px; font-size:11px; cursor:help; }
        .badge-warn{ background:#ffc107; color:#212529; padding:3px 8px; border-radius:10px; font-size:11px; }

        /* ── Résultat final ──────────────────────── */
        .imp-result-summary { display:flex; gap:20px; margin-bottom:20px; flex-wrap:wrap; }
        .res-stat { display:flex; flex-direction:column; align-items:center; padding:20px 30px;
                    border-radius:10px; font-size:14px; font-weight:600; gap:4px; flex:1; min-width:120px; }
        .res-stat.ok   { background:#d4edda; color:#155724; }
        .res-stat.warn { background:#fff3cd; color:#856404; }
        .res-stat strong { font-size:28px; line-height:1; }
        .imp-section { margin-top:16px; }
        .imp-section h5 { font-size:14px; font-weight:700; margin-bottom:8px; color:#495057; }
        .imp-success-msg { padding:14px 18px; background:#d4edda; color:#155724; border-radius:8px;
                           font-weight:600; margin-top:16px; }

        /* ── Boutons nav ─────────────────────────── */
        .imp-nav { display:flex; justify-content:space-between; align-items:center;
                   margin-top:24px; padding-top:16px; border-top:1px solid #dee2e6; }
        .btn-imp { display:inline-flex; align-items:center; gap:8px; padding:10px 22px;
                   border:none; border-radius:6px; font-size:14px; font-weight:600; cursor:pointer; }
        .btn-imp-prev  { background:#6c757d; color:#fff; }
        .btn-imp-next  { background:#007bff; color:#fff; }
        .btn-imp-launch{ background:#28a745; color:#fff; }
        .btn-imp-reset { background:#dc3545; color:#fff; }
        .btn-imp-tpl   { background:#17a2b8; color:#fff; }
