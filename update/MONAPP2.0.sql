...
BEGIN
    CREATE TABLE USERS (
        ...
        MENU_PERMISSIONS NVARCHAR(MAX) NULL,  -- Nouvelle colonne pour les permissions
...
BEGIN
    INSERT INTO MENUS (CODE, NOM, URL, ICONE, ORDRE, PARENT_ID) VALUES
    -- Menu principal
    ('dashboard', 'Dashboard', '/pages/accueil/dashboards/index.aspx', 'fas fa-chalkboard', 1, NULL),
    
    -- Modules
    ('modules', 'Modules', NULL, 'fas fa-cubes', 2, NULL),
    ('eleves', 'Liste des élèves', '/pages/modules/eleves/eleves.aspx', 'fas fa-users', 1, (SELECT ID FROM MENUS WHERE CODE = 'modules')),
    ('absences', 'Retards & Absences', '/pages/modules/absences/absences.aspx', 'fas fa-calendar-times', 2, (SELECT ID FROM MENUS WHERE CODE = 'modules')),
    ('frais', 'Frais scolaires', '/pages/modules/frais/frais.aspx', 'fas fa-money-bill-wave', 3, (SELECT ID FROM MENUS WHERE CODE = 'modules')),
    ('bulletins', 'Bulletins', '/pages/modules/bulletins/bulletins.aspx', 'fas fa-file-alt', 4, (SELECT ID FROM MENUS WHERE CODE = 'modules')),
    
    -- Paramètres
    ('parametres', 'Paramètres', NULL, 'fas fa-sliders-h', 3, NULL),
    ('niveaux', 'Niveaux', '/pages/parametres/niveaux/niveaux.aspx', 'fas fa-layer-group', 1, (SELECT ID FROM MENUS WHERE CODE = 'parametres')),
    ('salles', 'Salles', '/pages/parametres/salles/salles.aspx', 'fas fa-door-open', 2, (SELECT ID FROM MENUS WHERE CODE = 'parametres')),
    ('classes', 'Classes', '/pages/parametres/classes/classes.aspx', 'fas fa-folder', 3, (SELECT ID FROM MENUS WHERE CODE = 'parametres')),
    ('matieres', 'Matières', '/pages/parametres/matieres/matieres.aspx', 'fas fa-book', 4, (SELECT ID FROM MENUS WHERE CODE = 'parametres')),
    
    -- Administrations
    ('administrations', 'Administrations', NULL, 'fas fa-university', 4, NULL),
    ('utilitaires', 'Utilitaires', '/pages/administrations/utilitaires/utilitaires.aspx', 'fas fa-cogs', 1, (SELECT ID FROM MENUS WHERE CODE = 'administrations')),
    ('annee', 'Années', '/pages/administrations/annee/annee.aspx', 'fas fa-calendar-alt', 2, (SELECT ID FROM MENUS WHERE CODE = 'administrations')),
    ('utilisateur', 'Utilisateurs', '/pages/administrations/utilisateur/utilisateur.aspx', 'fas fa-user-shield', 3, (SELECT ID FROM MENUS WHERE CODE = 'administrations')),
    ('requetes', 'Requêtes SQL', '/pages/administrations/requete/requetes.aspx', 'fas fa-database', 4, (SELECT ID FROM MENUS WHERE CODE = 'administrations'));
END
GO
...
BEGIN
    CREATE TABLE BULLETINS (
        ...
        NOTE1 DECIMAL(4,1) NULL,
        NOTE2 DECIMAL(4,1) NULL,
        NOTE_PROJET DECIMAL(4,1) NULL,
...
BEGIN
    ...
    NOM NVARCHAR(100) NOT NULL,
    CLASSE INT NOT NULL,
    TYPE NVARCHAR(20) NOT NULL CHECK (TYPE IN ('absence', 'retard')),
    DATE_DEBUT DATETIME NOT NULL,
    DATE_FIN DATETIME NULL,
    HEURE_DEBUT TIME NULL,
    HEURE_FIN TIME NULL,
    DUREE DECIMAL(5,2) NULL,
    JUSTIF BIT NOT NULL DEFAULT 0,
    ...
    -- Index
    CREATE INDEX IX_ABSENCES_MATRICULE ON ABSENCES(MATRICULE);
    CREATE INDEX IX_ABSENCES_DATE_DEBUT ON ABSENCES(DATE_DEBUT);
